...
def main():
    ...
    deepseek_models = [r for r in data if r.get("display_org") == "DeepSeek"]
    
    print(f"DeepSeek Models: {len(deepseek_models)}")
    print(f"\nLicense Breakdown:")
    print("-" * 50)
    
    licenses = Counter(r.get("license", "Unknown") for r in deepseek_models)
    ...
    print(f"\nAll DeepSeek Models:")
    print("-" * 50)
    for model in sorted(deepseek_models, key=lambda x: x.get("model_id", "")):
